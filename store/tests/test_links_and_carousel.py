...
class TestLinksAndCarousel(TestCase):
            ...
            for a in soup.find_all("a", href=True):
                ...
                if href.startswith("/") and not href.startswith("//"):
                    ...
                    self.assertIn(
                        r.status_code,
                        (200, 301, 302),
                        f"Lien {href} status {r.status_code} depuis {path}",
                    )
            ...
            for img in soup.find_all("img", src=True):
                ...
                if src.startswith("/") and not src.startswith("//"):
                    ...
                    self.assertIn(
                        r.status_code,
                        (200, 304),
                        f"Image {src} status {r.status_code} depuis {path}",
                    )
    ...
    def test_carousel_accessibility(self):
        ...
        self.assertIsNotNone(
            carousel,
            "Carrousel non trouvé (x-data avec 'slides' ou classe contenant 'carousel'/'slider')",
        )
