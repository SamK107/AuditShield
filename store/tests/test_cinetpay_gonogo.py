...
class CinetpayGoNoGoTests(TestCase):
    ...
    def test_webhook_no_signature(self):
        p = Payment.objects.create(order_id="ORD-NOSIG", status="PENDING", amount=15000, currency="XOF")
    ...
    def test_webhook_invalid_signature(self):
        p = Payment.objects.create(order_id="ORD-BAD", status="PENDING", amount=15000, currency="XOF")
        body = make_payload(p.order_id)
        raw = raw_json_bytes(body)
        resp = self.client.post("/payment/callback/", data=raw, content_type="application/json", **{hdr_key(): "bad"})
        self.assertEqual(resp.status_code, 400)
    ...
    def test_webhook_valid_signature_check_ko(self, mock_deliver, mock_check):
        p = Payment.objects.create(order_id="ORD-KO", status="PENDING", amount=15000, currency="XOF")
        body = make_payload(p.order_id)
        raw = raw_json_bytes(body)
        sig = sig_hex(raw)
        resp = self.client.post("/payment/callback/", data=raw, content_type="application/json", **{hdr_key(): sig})
        self.assertEqual(resp.status_code, 200)
        p.refresh_from_db()
    ...
    def test_webhook_valid_signature_check_ok(self, mock_deliver, mock_check):
        p = Payment.objects.create(order_id="ORD-OK", status="PENDING", amount=15000, currency="XOF")
        body = make_payload(p.order_id)
        raw = raw_json_bytes(body)
        sig = sig_hex(raw)
        resp = self.client.post("/payment/callback/", data=raw, content_type="application/json", **{hdr_key(): sig})
    ...
    def test_webhook_idempotence(self, mock_deliver, mock_check):
        p = Payment.objects.create(order_id="ORD-IDEMPOT", status="PENDING", amount=15000, currency="XOF")
        body = make_payload(p.order_id)
        raw = raw_json_bytes(body)
        sig = sig_hex(raw)
        # Premier appel : livraison
        resp1 = self.client.post("/payment/callback/", data=raw, content_type="application/json", **{hdr_key(): sig})
        self.assertEqual(resp1.status_code, 200)
        # Deuxième appel : pas de double livraison
        resp2 = self.client.post("/payment/callback/", data=raw, content_type="application/json", **{hdr_key(): sig})
