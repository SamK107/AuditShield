...
def _post(path: str, json_payload: dict, timeout: int = 25) -> dict:
    ...
    try:
        ...
        if path == "/v2/payment":
            lvl = logger.error if r.status_code >= 400 else logger.info
            lvl("CINETPAY_INIT status=%s body=%s", r.status_code, r.text[:2000])
        elif path == "/v2/payment/check":
            lvl = logger.error if r.status_code >= 400 else logger.info
            lvl("CINETPAY_CHECK status=%s body=%s", r.status_code, r.text[:2000])
...
def init_payment(
    *,
    ...
    # ICI : NETTOYAGE DES None 
    payload = {k: v for k, v in payload.items() if v is not None}
    # (optionnel) si tu veux aussi éliminer les chaînes vides :
    # payload = {k: v for k, v in payload.items() if v not in (None, "")}

    logger.info("[CinetPay][init] tx=%s amount=%s %s channels=%s",
                transaction_id, amount_int, currency, ch)
...
def init_payment_auto(order, request) -> str:
    ...
    # Aligne transaction_id de l'Order sur ce que tu envoies à CinetPay (tx)
    if getattr(order, "transaction_id", None) != tx:
        order.transaction_id = tx
        try:
...
def _is_mock_enabled() -> bool:
    ...
    if str(mock_value).lower() in ("1", "true", "yes"):
        logger.warning("[CinetPay] Mode MOCK activé via CINETPAY_MOCK dans .env")
        return True
...
def init_payment_auto(order, request) -> str:  # type: ignore[override]
    ...
    logger.info(f"[CinetPay][init_payment_auto] Mode MOCK: {mock_enabled}, order_id={order.id}")
    ...
    if mock_enabled:
        ...
        logger.warning(f"[CINETPAY MOCK] init_payment_auto -> order_id={order.id} tx={tx} url={mock_url}")
        return mock_url
    
    # Mode PRODUCTION : utiliser l'API réelle
    logger.info(f"[CINETPAY] Mode RÉEL activé (pas de mock) -> order_id={order.id}")
