...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--url",
            default=DEFAULT_URL,
            help="URL du callback (defaut: http://127.0.0.1:8000/payment/callback/)",
        )
        parser.add_argument("--order", required=True, help="Order ID (ex: ORD-TEST-001)")
        parser.add_argument(
            "--status", choices=["PAID", "FAILED"], default="PAID", help="Statut simulé"
        )
        parser.add_argument(
            "--signature", choices=["ok", "bad"], default="ok", help="Signature HMAC simulée"
        )
        ...
        parser.add_argument("--amount", type=int, default=100, help="Montant simulé")
        ...
        parser.add_argument(
            "--algo", choices=["hex", "b64", "hexpref"], default="hex", help="HMAC signature format"
        )
    ...
    def handle(self, *args, **opts):
        ...
        for i in range(repeat):
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"[{i+1}/{repeat}] {resp.status_code} {resp.text[:200]}")
                )
