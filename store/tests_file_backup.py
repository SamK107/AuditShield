...
def test_start_checkout_creates_payment_and_redirects(client):
    ...
    from store.models import Product, OfferTier
    product = Product.objects.create(slug="test-prod", title="Test", price_fcfa=1000, is_published=True)
    tier = OfferTier.objects.create(product=product, kind="STANDARD", title="Standard", price_fcfa=1500)
    with patch("store.services.cinetpay.init_payment") as mock_init:
        mock_init.return_value = "https://pay.cinetpay.com/redirect"
        resp = client.post(reverse("store:start_checkout", args=[product.slug, tier.id]), {"email": "a@b.com"})
...
def test_webhook_no_signature_returns_400(client):
    ...
    resp = client.post(url, data=json.dumps({"transaction_id": "tx1"}), content_type="application/json")
...
def test_webhook_signature_ok_payment_check_ko(client, settings):
    ...
    settings.CINETPAY_WEBHOOK_SECRET = "testsecret"
    # Créer un Payment
    payment = Payment.objects.create(order_id="tx2", amount=1000, currency="XOF", email="t@t.com", status="PENDING")
...
def test_webhook_signature_ok_payment_check_ok_and_delivery(client, settings):
    ...
    url = reverse("store:cinetpay_callback")
    settings.CINETPAY_WEBHOOK_SECRET = "testsecret"
    payment = Payment.objects.create(order_id="tx3", amount=1000, currency="XOF", email="t@t.com", status="PENDING")
...
def test_webhook_idempotent(client, settings):
    ...
    url = reverse("store:cinetpay_callback")
    settings.CINETPAY_WEBHOOK_SECRET = "testsecret"
    payment = Payment.objects.create(order_id="tx4", amount=1000, currency="XOF", email="t@t.com", status="PAID")
